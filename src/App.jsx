...
function detectConflicts(grid,days,slots){
  ...
  classSlots.forEach(slot=>{
    ...
    days.forEach(day=>{
      ...
      if(cell){
        if(facultyInSlot[cell.faculty]){conflicts.push({slot:slot.label,faculty:cell.faculty,days:[facultyInSlot[cell.faculty],day]});}
        else facultyInSlot[cell.faculty]=day;
      }
    });
  });
  return conflicts;
    ...
    <div style={{fontFamily:"'Segoe UI',sans-serif",background:bg,minHeight:"100vh",color:text}}>
      <div style={{minHeight:"100vh",position:"relative",overflow:"hidden",display:"flex",flexDirection:"column",padding:"0 20px 60px"}}>
        ...
        <nav style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"20px 40px",borderBottom:"1px solid rgba(255,255,255,0.1)",position:"relative",zIndex:2}}>
          ...
          {registered&&<button onClick={onEnter} style={{background:"linear-gradient(135deg,#6366f1,#8b5cf6)",color:"#fff",border:"none",borderRadius:8,padding:"10px 24px",cursor:"pointer",fontWeight:600}}>Launch App →</button>}
        </nav>
        <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",position:"relative",zIndex:2}}>
          <div style={{textAlign:"center",maxWidth:680}}>
            ...
            {!registered?(
              <div style={{background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.12)",borderRadius:20,padding:32,backdropFilter:"blur(10px)"}}>
                <p style={{color:"#c084fc",fontWeight:700,fontSize:16,marginBottom:8}}>🚀 Register to Get Started — It's Free!</p>
                <p style={{color:"#94a3b8",fontSize:14,marginBottom:24}}>Join educators using Tymr to simplify academic scheduling.</p>
                <a href={REGISTER_URL} target="_blank" rel="noreferrer" onClick={()=>setRegistered(true)} style={{display:"inline-block",background:"linear-gradient(135deg,#6366f1,#8b5cf6)",color:"#fff",borderRadius:12,padding:"16px 48px",fontSize:18,fontWeight:700,textDecoration:"none",boxShadow:"0 0 40px rgba(99,102,241,0.5)"}}>📝 Register Free →</a>
                <p style={{color:"#475569",fontSize:12,marginTop:16}}>No credit card required • Free 14-day trial • Cancel anytime</p>
              </div>
            ):(
              <div style={{background:"rgba(16,185,129,0.1)",border:"1px solid rgba(16,185,129,0.3)",borderRadius:20,padding:32,backdropFilter:"blur(10px)"}}>
                <div style={{fontSize:48,marginBottom:12}}>🎉</div>
                <p style={{color:"#34d399",fontWeight:700,fontSize:20,marginBottom:8}}>Thank you for registering!</p>
                <p style={{color:"#94a3b8",fontSize:14,marginBottom:24}}>You're all set. Click below to start building your timetable.</p>
                <button onClick={onEnter} style={{background:"linear-gradient(135deg,#10b981,#059669)",color:"#fff",border:"none",borderRadius:12,padding:"16px 48px",fontSize:18,fontWeight:700,cursor:"pointer",boxShadow:"0 0 40px rgba(16,185,129,0.4)"}}>🚀 Launch Tymr App →</button>
              </div>
            )}
          </div>
        </div>
      ...
      <div style={{padding:"60px 20px",background:darkMode?"#0f0c29":"#fff"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))",gap:20,maxWidth:960,margin:"0 auto"}}>
          {[{name:"Free Trial",price:"₹0",period:"/14 days",color:"#64748b",features:["5 timetables","Basic attendance","2 exam schedules","Email support"]},{name:"Starter",price:"₹29",period:"/month",color:"#0ea5e9",features:["Unlimited timetables","Advanced attendance","10 exam schedules","Priority email","5 users","CSV/PDF export"]},{name:"Professional",price:"₹49",period:"/month",color:"#6366f1",popular:true,features:["Everything in Starter","Unlimited exams","Advanced seating","20 users","Custom branding","Analytics","API access","Phone support"]},{name:"Enterprise",price:"₹99",period:"/month",color:"#8b5cf6",features:["Everything in Pro","Unlimited users","Dedicated manager","Custom integrations","On-premise","24/7 support","Training"]}].map(p=>(
            <div key={p.name} style={{background:p.popular?(darkMode?"linear-gradient(135deg,rgba(99,102,241,0.2),rgba(139,92,246,0.2))":"linear-gradient(135deg,#eef2ff,#f5f3ff)"):(darkMode?"rgba(255,255,255,0.04)":"#f8fafc"),border:`2px solid ${p.popular?"#6366f1":(darkMode?"rgba(255,255,255,0.08)":"#e2e8f0")}`,borderRadius:16,padding:24,position:"relative"}}>
              ...
              <a href={p.price==="₹0"?REGISTER_URL:PAYMENT_URL} target="_blank" rel="noreferrer" style={{display:"block",textAlign:"center",background:`linear-gradient(135deg,${p.color},${p.color}aa)`,color:"#fff",borderRadius:8,padding:"10px",textDecoration:"none",fontWeight:600,fontSize:14}}>{p.price==="₹0"?"Register Free":"Subscribe Now"}</a>
            </div>
          ))}
        </div>
      </div>
      {/* Footer */}
      <footer style={{background:darkMode?"#020617":"#1e293b",color:"#94a3b8",padding:"40px 20px",borderTop:"1px solid rgba(255,255,255,0.06)"}}>
        <div style={{maxWidth:960,margin:"0 auto",display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))",gap:32}}>
          <div>
            <div style={{fontSize:22,fontWeight:800,background:"linear-gradient(90deg,#818cf8,#c084fc)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",marginBottom:12}}>Tymr</div>
            <p style={{fontSize:14}}>Smart academic scheduling for modern institutions.</p>
          </div>
          <div>
            <div style={{fontWeight:700,marginBottom:12,color:"#e2e8f0"}}>Contact Us</div>
            <div style={{fontSize:14,lineHeight:2}}>📞 +91 97908 18436<br/>✉️ acadcoachingcenter@gmail.com<br/>📍 DABC-BEGONIA, Mambakkam<br/>&nbsp;&nbsp;&nbsp;(Near VIT Chennai Campus)<br/>&nbsp;&nbsp;&nbsp;Chennai - TN - 600127</div>
          </div>
          <div>
            <div style={{fontWeight:700,marginBottom:12,color:"#e2e8f0"}}>Quick Links</div>
            <div style={{fontSize:14,lineHeight:2,cursor:"pointer"}} onClick={onEnter}>Timetable Maker<br/>Exam Scheduler<br/>Attendance Tracker</div>
          </div>
        </div>
...
export default function App(){
  ...
  function handleCSV(e){
    ...
    reader.onload=ev=>{try{const p=parseCSV(ev.target.result);if(!p.length){setCsvError("No valid rows found.");return;}setSubjects(p);setCsvError("");setGenerated(false);setTimetable(null);}catch{setCsvError("Failed to parse CSV.");}};
    reader.readAsText(file);
  }

  function downloadTemplate(){
    const csv="Faculty Name,Subject Name,Relevance Score,Hours Per Week,Type (T/P),Consecutive Periods (Lab only),Room/Hall\nDr. Smith,Mathematics,5,4,T,,Room 101\nProf. Jones,Physics Lab,4,3,P,3,Lab A\nDr. Patel,Chemistry,3,3,T,,Room 202\n";
  ...
  function handleGenerate(){
    ...
    const c=detectConflicts(grid,days,slots);
    setConflicts(c);
    setTimetable({grid,days,slots});
    setGenerated(true);
    if(!isRegistered)setTrialCount(ct=>ct+1);
  }
  ...
  function handleDrop(day,slotIdx){
    ...
    const newTT={...timetable,grid:newGrid};
    setTimetable(newTT);
  ...
  function saveTimetable(){
    ...
    const entry={name:saveName,timetable,subjects,config,date:new Date().toLocaleDateString()};
    setSavedTimetables(prev=>{const updated=[...prev.filter(t=>t.name!==saveName),entry];return updated;});
    setSaveName("");
  }
  function loadTimetable(entry){setTimetable(entry.timetable);setSubjects(entry.subjects);setConfig(entry.config);setGenerated(true);setConflicts(detectConflicts(entry.timetable.grid,entry.timetable.days,entry.timetable.slots));}

  function downloadCSV(){
    if(!timetable)return;
    const{grid,days,slots:sl}=timetable;
    const header=["Day",...sl.map(s=>s.label)].join(",");
    const rows=days.map(d=>{const cells=sl.map(s=>{if(s.type==="tea")return"Tea Break";if(s.type==="lunch")return"Lunch Break";const c=grid[d][s.index];return c?`"${c.subject}(${c.faculty})${c.room?` [${c.room}]`:""}"`:"";});return[d,...cells].join(",");});
  ...
  function downloadPDF(){
    if(!timetable)return;
    const{grid,days,slots:sl}=timetable;
    let html=`<html><head><style>body{font-family:Arial,sans-serif;font-size:11px;margin:20px}h2{color:#4f46e5;text-align:center}table{width:100%;border-collapse:collapse}th,td{border:1px solid #ccc;padding:6px 8px;text-align:center}th{background:#4f46e5;color:#fff}.tea{background:#fef3c7}.lunch{background:#d1fae5}.lab{background:#ede9fe}.empty{color:#999}</style></head><body><h2>Tymr – Generated Timetable</h2><table><tr><th>Day</th>${sl.map(s=>`<th>${s.label}</th>`).join("")}</tr>`;
    days.forEach(d=>{html+=`<tr><td><b>${d}</b></td>`;sl.forEach(s=>{if(s.type==="tea")html+=`<td class="tea">☕ Tea</td>`;else if(s.type==="lunch")html+=`<td class="lunch">🍽 Lunch</td>`;else{const c=grid[d][s.index];html+=c?`<td class="${c.isLab?"lab":""}">${c.subject}<br/><small>${c.faculty}</small>${c.room?`<br/><small>[${c.room}]</small>`:""}</td>`:`<td class="empty">—</td>`;}});html+=`</tr>`;});
    html+=`</table></body></html>`;
    const w=window.open("","_blank");w.document.write(html);w.document.close();w.print();
  }

  function downloadFacultyPDF(faculty){
    if(!timetable)return;
    const{grid,days,slots:sl}=timetable;
    let html=`<html><head><style>body{font-family:Arial,sans-serif;font-size:12px;margin:20px}h2{color:#4f46e5}h3{color:#666}table{width:100%;border-collapse:collapse}th,td{border:1px solid #ccc;padding:8px;text-align:center}th{background:#4f46e5;color:#fff}.active{background:#ede9fe;font-weight:bold}.empty{color:#999}</style></head><body><h2>Tymr – Faculty Timetable</h2><h3>${faculty}</h3><table><tr><th>Day</th>${sl.filter(s=>s.type==="class").map(s=>`<th>${s.label}</th>`).join("")}</tr>`;
    days.forEach(d=>{html+=`<tr><td><b>${d.slice(0,3)}</b></td>`;sl.filter(s=>s.type==="class").forEach(s=>{const c=grid[d][s.index];if(c&&c.faculty===faculty)html+=`<td class="active">${c.subject}${c.room?`<br/>[${c.room}]`:""}</td>`;else html+=`<td class="empty">—</td>`;});html+=`</tr>`;});
    html+=`</table></body></html>`;
    const w=window.open("","_blank");w.document.write(html);w.document.close();w.print();
  }
  ...
  function shareWhatsApp(){
    ...
    const text=`📅 *Tymr Timetable*\nDays: ${days.join(", ")}\nGenerated via tymr-two.vercel.app\n\n_Register free at ${REGISTER_URL}_`;
  ...
  if(page==="landing")return <LandingPage onEnter={()=>setPage("app")} darkMode={darkMode}/>;

  const navItems=[{id:"timetable",label:"📅 Timetable"},{id:"exam",label:"📝 Exam Schedule"},{id:"attendance",label:"✅ Attendance"}];
  ...
  return(
    <div style={{fontFamily:"'Segoe UI',sans-serif",minHeight:"100vh",background:bg,color:text,display:"flex",flexWrap:"wrap"}}>
      {/* Sidebar */}
      <aside style={{width:220,background:sideBg,borderRight:`1px solid ${border}`,padding:"24px 0",display:"flex",flexDirection:"column",flexShrink:0,minHeight:"100vh"}}>
        ...
        {navItems.map(n=>(
          <div key={n.id} onClick={()=>setTab(n.id)} style={{padding:"12px 20px",cursor:"pointer",background:tab===n.id?(darkMode?"rgba(99,102,241,0.15)":"#eef2ff"):"transparent",borderLeft:tab===n.id?"3px solid #6366f1":"3px solid transparent",color:tab===n.id?"#818cf8":muted,fontWeight:tab===n.id?600:400}}>{n.label}</div>
        ))}
        <div style={{marginTop:"auto",padding:"16px 20px",borderTop:`1px solid ${border}`}}>
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
            ...
            <div onClick={()=>setDarkMode(d=>!d)} style={{width:44,height:24,background:darkMode?"#6366f1":"#cbd5e1",borderRadius:12,cursor:"pointer",position:"relative",transition:"background 0.3s"}}>
          ...
          <div style={{fontSize:12,color:muted,marginBottom:8}}>Free Trial {!isRegistered&&`(${TRIAL_LIMIT-trialCount} left)`}</div>
          <a href={REGISTER_URL} target="_blank" rel="noreferrer" onClick={()=>setIsRegistered(true)} style={{display:"block",textAlign:"center",background:"rgba(99,102,241,0.1)",color:"#818cf8",border:"1px solid #6366f1",borderRadius:8,padding:"8px",textDecoration:"none",fontSize:13,fontWeight:600,marginBottom:8}}>📝 Register</a>
          <a href={PAYMENT_URL} target="_blank" rel="noreferrer" style={{display:"block",textAlign:"center",background:"linear-gradient(135deg,#6366f1,#8b5cf6)",color:"#fff",borderRadius:8,padding:"8px",textDecoration:"none",fontSize:13,fontWeight:600}}>Upgrade →</a>
        </div>
      </aside>
      ...
      <main style={{flex:1,padding:24,overflowY:"auto",minWidth:0}}>
          ...
          <div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(300px,1fr))",gap:20,marginBottom:24}}>
                ...
                {subjects.length>0&&(
                  <div style={{marginTop:12}}>
                    ...
                    <div style={{maxHeight:160,overflowY:"auto"}}>
                      {subjects.map((s,i)=>(
                        <div key={i} style={{display:"flex",alignItems:"center",gap:8,padding:"4px 0",borderBottom:`1px solid ${border}`,fontSize:12}}>
                          <div style={{width:8,height:8,borderRadius:"50%",background:subjectColors[s.subject],flexShrink:0}}/>
                          <span style={{flex:1,fontWeight:600}}>{s.subject}</span>
                          <span style={{color:muted}}>{s.faculty}</span>
                          {s.room&&<span style={{color:muted,fontSize:10}}>[{s.room}]</span>}
                          <span style={{background:s.type==="P"?"#ede9fe22":"#dbeafe22",color:s.type==="P"?"#a78bfa":"#60a5fa",borderRadius:4,padding:"1px 5px",fontSize:10}}>{s.type==="P"?"Lab":"Theory"}</span>
                        </div>
                      ))}
                    </div>
                  </div>
              ...
              <div style={{background:cardBg,borderRadius:16,padding:24,border:`1px solid ${border}`}}>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                  {[["Start Time","start","time"],["End Time","end","time"],["Period (mins)","period","number"]].map(([lbl,key,type])=>(
                    <div key={key}>
                      <label style={{fontSize:12,color:muted,display:"block",marginBottom:4}}>{lbl}</label>
                      <input type={type} value={config[key]} onChange={e=>setConfig(c=>({...c,[key]:type==="number"?parseInt(e.target.value)||1:e.target.value}))} style={{width:"100%",background:inputBg,border:`1px solid ${border}`,borderRadius:8,padding:"8px 10px",color:text,fontSize:13,boxSizing:"border-box"}}/>
                    </div>
                  ))}
                ...
                <div style={{marginTop:12,background:inputBg,borderRadius:10,padding:12}}>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                    {[["From","teaStart"],["To","teaEnd"]].map(([lbl,key])=>(
                      <div key={key}><label style={{fontSize:11,color:muted,display:"block",marginBottom:3}}>{lbl}</label>
                      <input type="time" value={config[key]} onChange={e=>setConfig(c=>({...c,[key]:e.target.value}))} style={{width:"100%",background:cardBg,border:`1px solid ${border}`,borderRadius:8,padding:"7px 10px",color:text,fontSize:13,boxSizing:"border-box"}}/></div>
                    ))}
                  </div>
                  <div style={{fontSize:11,color:muted,marginTop:4}}>Duration: {parseTime(config.teaEnd)-parseTime(config.teaStart)} mins</div>
                </div>
                <div style={{marginTop:10,background:inputBg,borderRadius:10,padding:12}}>
                  ...
                  <div style={{fontSize:11,color:muted,marginTop:4}}>Duration: {parseTime(config.lunchEnd)-parseTime(config.lunchStart)} mins</div>
                </div>
              </div>
            </div>
            ...
            {!isRegistered&&(
              <div style={{marginBottom:16,display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
                <div style={{background:"rgba(99,102,241,0.1)",border:"1px solid #6366f1",borderRadius:10,padding:"8px 16px",fontSize:13,color:"#a5b4fc"}}>🎯 Free Trials: <strong>{TRIAL_LIMIT-trialCount}</strong>/{TRIAL_LIMIT} remaining</div>
                <a href={REGISTER_URL} target="_blank" rel="noreferrer" onClick={()=>setIsRegistered(true)} style={{background:"rgba(16,185,129,0.1)",border:"1px solid #10b981",borderRadius:10,padding:"8px 16px",fontSize:13,color:"#34d399",textDecoration:"none",fontWeight:600}}>✅ Register to Unlock Unlimited</a>
              </div>
            )}
            ...
            {!isRegistered&&trialCount>=TRIAL_LIMIT?(
              <div style={{background:darkMode?"rgba(15,12,41,0.97)":"#f1f5f9",border:"2px solid #6366f1",borderRadius:16,padding:40,textAlign:"center",marginBottom:24}}>
                ...
                <p style={{color:muted,marginBottom:28}}>Register free to unlock unlimited timetable generation!</p>
                <a href={REGISTER_URL} target="_blank" rel="noreferrer" onClick={()=>setIsRegistered(true)} style={{display:"inline-block",background:"linear-gradient(135deg,#6366f1,#8b5cf6)",color:"#fff",borderRadius:12,padding:"14px 40px",fontSize:16,fontWeight:700,textDecoration:"none"}}>📝 Register Free to Continue →</a>
              </div>
            ):(
            ...
            {conflicts.length>0&&(
              <div style={{background:"rgba(239,68,68,0.1)",border:"1px solid #ef4444",borderRadius:12,padding:16,marginBottom:20}}>
                ...
                {conflicts.map((c,i)=>(
                  <div key={i} style={{fontSize:13,color:muted,marginBottom:4}}>• <strong>{c.faculty}</strong> is double-booked at <strong>{c.slot}</strong> on {c.days.join(" & ")}</div>
                ))}
              </div>
            )}
            ...
            {generated&&timetable&&(
              <div>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12,flexWrap:"wrap",gap:8}}>
                  <h3 style={{fontWeight:700}}>Generated Timetable <span style={{fontSize:13,color:muted,fontWeight:400}}>(drag cells to swap)</span></h3>
                ...
                {uniqueFaculty.length>0&&(
                  <div style={{marginBottom:16}}>
                    <label style={{fontSize:13,color:muted,marginRight:8}}>🧑‍🏫 Print Faculty Timetable:</label>
                    ...
                    {facultyView&&<button onClick={()=>downloadFacultyPDF(facultyView)} style={{marginLeft:8,background:"#8b5cf6",color:"#fff",border:"none",borderRadius:8,padding:"6px 14px",cursor:"pointer",fontSize:13,fontWeight:600}}>🖨 Print</button>}
                  </div>
                )}
                    ...
                    <thead>
                      <tr>
                        ...
                        {timetable.slots.map((s,i)=>(
                          <th key={i} style={{background:s.type==="tea"?"#78350f44":s.type==="lunch"?"#14532d44":(darkMode?"#1e3a5f":"#eff6ff"),border:`1px solid ${border}`,padding:"8px 10px",minWidth:90,whiteSpace:"nowrap",color:s.type==="tea"?"#fbbf24":s.type==="lunch"?"#34d399":(darkMode?"#93c5fd":"#1d4ed8"),fontSize:11}}>{s.label}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {timetable.days.map(day=>(
                        <tr key={day}>
                          ...
                          {timetable.slots.map((s,i)=>{
                            ...
                            const bg=cell?(subjectColors[cell.subject]+"22"):(darkMode?"transparent":"#fafafa");
                            const fg=cell?subjectColors[cell.subject]:(darkMode?"#334155":"#cbd5e1");
                            return(
                              <td key={i} draggable onDragStart={()=>handleDragStart(day,s.index)} onDragOver={e=>e.preventDefault()} onDrop={()=>handleDrop(day,s.index)}
                                style={{border:`1px solid ${isConflict?"#ef4444":border}`,padding:"6px 8px",background:bg,textAlign:"center",cursor:"grab",outline:isConflict?"2px solid #ef4444":"none",minWidth:90}}>
                                {cell?(
                                  <>
                                    <div style={{fontWeight:700,color:fg,fontSize:11}}>{cell.subject}</div>
                                    <div style={{color:muted,fontSize:10}}>{cell.faculty}</div>
                                    {cell.room&&<div style={{color:muted,fontSize:9}}>[{cell.room}]</div>}
                                    {cell.isLab&&<div style={{color:"#a78bfa",fontSize:9}}>🔬 Lab</div>}
                                  </>
                                ):<span style={{color:fg,fontSize:16}}>—</span>}
                              </td>
                            );
                          })}
                        </tr>
                ...
                <div style={{marginTop:16,display:"flex",flexWrap:"wrap",gap:8}}>
                  {subjects.map(s=>(
                    <div key={s.subject} style={{display:"flex",alignItems:"center",gap:6,background:cardBg,border:`1px solid ${border}`,borderRadius:20,padding:"4px 12px",fontSize:12}}>
                ...
                <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:12}}>
                  {savedTimetables.map((t,i)=>(
                    <div key={i} style={{background:cardBg,border:`1px solid ${border}`,borderRadius:12,padding:16}}>
                      ...
                      <button onClick={()=>loadTimetable(t)} style={{background:"linear-gradient(135deg,#6366f1,#8b5cf6)",color:"#fff",border:"none",borderRadius:8,padding:"6px 14px",cursor:"pointer",fontSize:13,fontWeight:600}}>📂 Load</button>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div style={{background:cardBg,borderRadius:16,padding:24,border:`1px solid ${border}`,marginBottom:24,maxWidth:700}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
                {[["Exam Title","title","text"],["Date","date","date"],["Time","time","time"],["Duration (hrs)","duration","number"],["Exam Hall","hall","text"]].map(([lbl,key,type])=>(
                  <div key={key}>
                    <label style={{fontSize:12,color:muted,display:"block",marginBottom:4}}>{lbl}</label>
                    <input type={type} value={examForm[key]} onChange={e=>setExamForm(f=>({...f,[key]:e.target.value}))} style={{width:"100%",background:inputBg,border:`1px solid ${border}`,borderRadius:8,padding:"8px 10px",color:text,fontSize:13,boxSizing:"border-box"}}/>
                  </div>
                ))}
                <div>
                  <label style={{fontSize:12,color:muted,display:"block",marginBottom:4}}>Seating</label>
            ...
            {exams.length>0&&(
              <div>
                ...
                <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))",gap:16}}>
                  {exams.map(ex=>(
                    <div key={ex.id} style={{background:cardBg,border:`1px solid ${border}`,borderRadius:16,padding:20}}>
        ...
        {tab==="exam"&&viewExam&&(
          <div>
            ...
            <div style={{background:cardBg,borderRadius:16,padding:24,border:`1px solid ${border}`,maxWidth:900}}>
              ...
              <div style={{background:inputBg,borderRadius:8,padding:"8px",textAlign:"center",color:muted,fontSize:12,marginBottom:16,letterSpacing:4}}>── FRONT / STAGE ──</div>
              <div style={{overflowX:"auto"}}>
                    ...
                    {Array.from({length:viewExam.rows},(_,r)=>(
                      <tr key={r}>
                        {Array.from({length:viewExam.cols},(_,c)=>{
                          ...
                          let occupied=viewExam.seating==="consecutive"?true:viewExam.seating==="alternate"?(r+c)%2===0:Math.random()>0.3;
                          return(
                            <td key={c} style={{border:`1px solid ${border}`,width:60,height:38,textAlign:"center",fontSize:10,background:occupied?"rgba(99,102,241,0.2)":inputBg,color:occupied?"#818cf8":muted,borderRadius:4}}>
                              {occupied?viewExam.rolls[idx]:"—"}
                            </td>
                          );
                        })}
                      </tr>
              ...
              <div style={{marginTop:16,display:"flex",gap:16,fontSize:13}}>
                ...
                <div style={{display:"flex",alignItems:"center",gap:6}}><div style={{width:14,height:14,background:inputBg,border:`1px solid ${border}`,borderRadius:2}}/> Empty</div>
              </div>
            </div>
          </div>
        )}
        ...
        {tab==="attendance"&&(
          <div>
            ...
            <p style={{color:muted,marginBottom:32}}>Track student attendance per class.</p>
            <div style={{background:cardBg,borderRadius:16,padding:40,border:`1px solid ${border}`,textAlign:"center",maxWidth:500}}>
